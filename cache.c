...
unsigned long get_cache_block_addr(cache_t *cache, unsigned long addr) {
  ...
  int bitmask = 1;
  bitmask = bitmask << (cache->n_offset_bit - 1);
  bitmask = ~bitmask;
  return (bitmask & addr);
}
 ...
 */
bool no_protocol_helper(cache_t *cache, unsigned long addr, enum action_t action) {
...
bool vi_protocol_helper(cache_t *cache, unsigned long addr, enum action_t action) {
  unsigned long addr_tag = get_cache_tag(cache, addr);
  unsigned long index = get_cache_index(cache, addr);
  int lru = cache->lru_way[index];
  bool hit = false;
...
bool access_cache(cache_t *cache, unsigned long addr, enum action_t action) {
  ...
  if (cache->protocol == NONE)  {
    no_protocol_helper(cache,addr,action);
  } else if (cache->protocol == VI) {
    vi_protocol_helper(cache,addr,action);
  }
